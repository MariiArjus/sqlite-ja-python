...
def loe_andmed():
    """Täidab listboxi andmetega."""
    lb.delete(0, tk.END)
    conn = sqlite3.connect('filmid.db')
    c = conn.cursor()
    c.execute("SELECT * FROM filmid")
    for rida in c.fetchall():
        lb.insert(tk.END, f"{rida[0]} | {rida[1]} | {rida[2]}")
    conn.close()

def vali_kirje(event):
    """Paneb valitud rea andmed tekstikastidesse."""
    try:
        valitud = lb.get(lb.curselection())
        id_numbri = valitud.split(" | ")[0] # Võtame rea algusest ID
        
        conn = sqlite3.connect('filmid.db')
        c = conn.cursor()
        c.execute("SELECT * FROM filmid WHERE id=?", (id_numbri,))
        kirje = c.fetchone()
        conn.close()

        # Täidame väljad
        ent_id.config(state='normal') # Teeme ID välja korraks aktiivseks
        ent_id.delete(0, tk.END)
        ent_id.insert(0, kirje[0])
        ent_id.config(state='readonly') # ID-d ei tohi kasutaja ise muuta
...
def uuenda_andmeid():
    ...
    if not id_muudetav:
        messagebox.showwarning("Viga", "Vali esmalt kirje, mida muuta!")
        return

    conn = sqlite3.connect('filmid.db')
    c = conn.cursor()
    c.execute("UPDATE filmid SET pealkiri=?, aasta=? WHERE id=?", 
              (uus_pealkiri, uus_aasta, id_muudetav))
    conn.commit()
    conn.close()
    
    messagebox.showinfo("Edu", "Andmed uuendatud!")
    loe_andmed() # Värskendame nimekirja

# --- GUI ---
root = tk.Tk()
root.title("Andmete muutmine")

# Listbox kirje valimiseks
lb = tk.Listbox(root, width=40)
lb.grid(row=0, column=0, columnspan=2, padx=10, pady=10)
lb.bind('<<ListboxSelect>>', vali_kirje) # Seob klikkimise funktsiooniga

# Sisestusväljad
tk.Label(root, text="ID (lukus):").grid(row=1, column=0)
ent_id = tk.Entry(root, state='readonly') # ID-d ei muudeta käsitsi
...
tk.Label(root, text="Aasta:").grid(row=3, column=0)
ent_aasta = tk.Entry(root)
ent_aasta.grid(row=3, column=1)

# Nupud
tk.Button(root, text="Uuenda kirjet", command=uuenda_andmeid).grid(row=4, column=0, columnspan=2, pady=10)

loe_andmed() # Laeb andmed käivitamisel
root.mainloop()